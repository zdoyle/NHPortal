...
{
    public partial class InqInspectionBySticker : NHPortal.Classes.PortalPage
    {
        string strVin;
        string strVinDate;
        string strVinTime;
        ...
        {
            RedirectOnInvalidPermission(UserPermissions.InspectionInquiriesBySticker);
            if (IsPostBack)
            {
                ProcessPostBack();
            }
        ...
        {
            Master.SetHeaderText("Inspection History by Sticker");
            ddlStickerType.Items.Add("A");
            ddlStickerType.Items.Add("N");
        }

        private void ProcessPostBack()
        {
            string action = Master.MasterPage.HidActionValue;
            tbSeries.Text = tbSeries.Text.PadLeft(2, '0');
            tbSticker.Text = tbSticker.Text.PadLeft(10, '0');
            switch (action)
            {
                case "RUN_REPORT":
                    RunReport();
        ...
        private void SetMetaData()
        {
            ...
            {
                Master.UserReport.MetaData.Add("Type", ddlStickerType.Text);
                Master.UserReport.MetaData.Add("Series", tbSeries.Text);
                Master.UserReport.MetaData.Add("Sticker", tbSticker.Text);
            }
        }
        ...
        private void LoadFavorite()
        {
            ...
            if (fav != null)
            {
                foreach (var c in fav.Criteria)
                {
                    ...
                    {
                        case "TYPE":
                            ddlStickerType.Text = c.Value;
                            break;

                        case "SERIES":
                            tbSeries.Text = c.Value;
                            break;

                        case "STICKER":
                            tbSticker.Text = c.Value;
                            break;
                    }
                }
            }
        ...
        {
            Master.SaveFavorite(NHPortal.UserControls.RedirectCodes.REPORT_INQ_INSPECTION_BY_STICKER, UserFavoriteTypes.Report);
        }

        private void RunReport()
        {
            List<OracleParameter> paramList = new List<OracleParameter>();
            paramList.Add(new OracleParameter("iType", OracleDbType.Varchar2, 1, ddlStickerType.Text, ParameterDirection.Input));
            paramList.Add(new OracleParameter("iSeries", OracleDbType.Varchar2, 2, tbSeries.Text, ParameterDirection.Input));
            paramList.Add(new OracleParameter("iSticker", OracleDbType.Varchar2, 10, tbSticker.Text, ParameterDirection.Input));
            paramList.Add(new OracleParameter("RECORD_DATA", OracleDbType.RefCursor, ParameterDirection.Output));

            OracleResponse response = ODAP.GetDataTableFromProcedure("NH_STIK_HISTORY", paramList.ToArray(), PortalFramework.Database.DatabaseTarget.OLTP);
            if (response.Successful)
            {
                LogMessage("Run Report - Response Successful", LogSeverity.Information);
                Master.UserReport = new Report("Inspection History by Sticker");
                SetMetaData();
                ...
                if (response.HasResults)
                {
                    ...
                    SetColumnTypes();
                    Master.UserReport.Sortable = false;
                    Master.UserReport.FooterVisibility = FooterVisibility.Auto;
                }
            }
            else
            {
                LogMessage("Run Reports - Response UnSuccessful", LogSeverity.Information);
        ...
        private void RunDetailsReport()
        {
            ...
            if (response.Successful)
            {
                LogMessage("Run Details - Response Successful", LogSeverity.Information);
                Master.UserReport = new Report("Inspection Inquiry By Sticker", response.ResultsTable);
                SetMetaData();
                Master.SetDataLastUpdate(DateTime.Now);         
                if (response.HasResults)
                {
                    DataRow row = response.ResultsTable.Rows[0];
                    LogMessage("Run Details - Response HasResults", LogSeverity.Information);
                    Master.UserReport.Columns.Insert("Details", "Select", 0);
                    AddDetailSelectColumnD2();
                    FormatReportValues();
                    SetColumnTypes();
                    Master.UserReport.Sortable = true;
                    Master.UserReport.FooterVisibility = FooterVisibility.Auto;

                    strVinDate = GDCoreUtilities.StringUtilities.GetSubstring(response.ResultsTable.Rows[0][1].ToString(), 0, 8);
                    strVinTime = GDCoreUtilities.StringUtilities.GetSubstring(response.ResultsTable.Rows[0][1].ToString(), 8, 6);

                    RunDetailsReportLastInfo();
                }
            }
            else
            {
                LogMessage("Run Details - Response UnSuccessful", LogSeverity.Information);
                Master.UserReport = null;
                Master.SetError(response.Exception);
            }
        ...
        private void RunDetailsReportLastInfo()
        {
            string qry = BuildDetailsQueryDetails();
            GDDatabaseClient.Oracle.OracleResponse response = PortalFramework.Database.ODAP.GetDataTable(qry, PortalFramework.Database.DatabaseTarget.OLTP);
            if (response.Successful)
            {
                if (response.HasResults)
        ...
        private void RunDetailsReportOverlay()
        {
            GetVIN();
            string qry = BuildDetailsQueryDetails();
        ...
        private void AddColumnsToReport()
        {
            if (Master.UserReport != null)
            {
                Master.UserReport.Columns.Add("Misc Date", "Date");
                Master.UserReport.Columns.Add("Purchased By", "Purchased By", ColumnDataType.Number);
                Master.UserReport.Columns.Add("Issued By", "Issued By", ColumnDataType.Percentage);
                Master.UserReport.Columns.Add("Issued To VIN", "Issued to VIN", ColumnDataType.Number);
                Master.UserReport.Columns.Add("Returned By", "Returned By", ColumnDataType.Percentage);
                Master.UserReport.Columns.Add("Replacement STicker", "Replacement Sticker", ColumnDataType.Number);
            }
        }
        ...
        private void AddDataToReport(DataTable dt)
        {
            ...
            strVin = dt.Rows[0][10].ToString();
            ReportRow rptRow = new ReportRow(Master.UserReport);
            NHPortal.Classes.NHPortalUtilities.TryConvertDateTimeFormat(dt.Rows[0][3].ToString(), "yyyyMMdd", "M/d/yyyy", out fmtDate);
            rptRow.Cells.Add(fmtDate.ToString());
            rptRow.Cells.Add(dt.Rows[0][0].ToString());
            rptRow.Cells.Add("-");
            rptRow.Cells.Add(dt.Rows[0][10].ToString());
            rptRow.Cells.Add("-");
            rptRow.Cells.Add("-");
            Master.UserReport.Rows.Add(rptRow);

            issueDate = dt.Rows[0][3].ToString();
            if (issueDate[0] != ' ')
            {
                rptRow = new ReportRow(Master.UserReport);
                rptRow.Cells.Add(fmtDate.ToString());
                rptRow.Cells.Add("-");
                rptRow.Cells.Add(dt.Rows[0][2].ToString());
                rptRow.Cells.Add(dt.Rows[0][10].ToString());
                rptRow.Cells.Add("-");
                rptRow.Cells.Add("-");
                Master.UserReport.Rows.Add(rptRow);
            ...
            replaceDate = dt.Rows[0][6].ToString();
            if (replaceDate[0] != ' ')
            {
                rptRow = new ReportRow(Master.UserReport);
                NHPortal.Classes.NHPortalUtilities.TryConvertDateTimeFormat(dt.Rows[0][6].ToString(), "yyyyMMdd", "M/d/yyyy", out fmtDate);
                rptRow.Cells.Add(fmtDate.ToString());
                rptRow.Cells.Add("-");
                rptRow.Cells.Add("-");
                rptRow.Cells.Add(dt.Rows[0][10].ToString());
                rptRow.Cells.Add(dt.Rows[0][5].ToString());
                rptRow.Cells.Add(dt.Rows[0][7].ToString() + "-" + dt.Rows[0][8].ToString() + "-" + dt.Rows[0][9].ToString());
                Master.UserReport.Rows.Add(rptRow);
            }
        }
        ...
        private string BuildDetailsQuery()
        {
            ...
            sb.AppendLine("     WHERE T.VEHICLESEQUENCENUM = (SELECT V.VEHICLESEQUENCENUMBER ");
            sb.AppendLine("                                     FROM VEHICLEMASTERTBL V ");
            sb.AppendLine("                                    WHERE V.VIN = '" + strVin + "') ");
        ...
        private void GetVIN()
        {
            ...
            {
                strVin = response.ResultsTable.Rows[0][10].ToString();
                strVinDate = response.ResultsTable.Rows[0][3].ToString();
                strVinTime = response.ResultsTable.Rows[0][4].ToString();
            }
            else
            {
        ...
        private void FormatReportValues()
        {
            ...
            foreach (ReportRow row in Master.UserReport.Rows)
            {
                ...
                row["Safety Result"].Value = Classes.Reference.InqInspectionBy.GetOverallResultDescription(row["Safety Result"].Value[0]);
                row["Emis Result"].Value = Classes.Reference.InqInspectionBy.GetOverallResultDescription(row["Emis Result"].Value[0]);
        ...
        private string BuildDetailsQueryDetails()
        {
            StringBuilder builder = new StringBuilder();

            builder = Classes.Reference.InqInspectionBy.InqInspectionBySQLbyOLTP(builder);
            builder.AppendLine("    WHERE T.VIN = '" + strVin + "' ");
            builder.AppendLine("      AND T.TESTDATE = '" + strVinDate + "' ");
            builder.AppendLine("      AND T.TESTENDTIME = '" + strVinTime + "' ");
        ...
        private void AddDetailSelectColumnD2()
        {
            int i = 0;
            foreach (ReportRow row in Master.UserReport.Rows)
            {
                row[0].Value = "Select";
                row[0].OnClick = String.Format("javascript: selectRowD2({0})", i);
                row[0].Href = "#/";
                i += 1;
            }
        }
