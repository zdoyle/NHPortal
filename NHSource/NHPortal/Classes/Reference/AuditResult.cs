...
namespace NHPortal.Classes.Reference
{
    ...
    public class AuditResultTypes
    {
        /// <summary>Gets the emission types from the database to store in memory.</summary>
        public static void Initialize()
        {
            string qry = "SELECT ret.AUDIT_RESULT_CODE, ret.AUDIT_RESULT_DESC" + Environment.NewLine
                       + "FROM   r_audit_result_type ret";

            List<AuditResultType> auditResultTypes = new List<AuditResultType>();
            OracleResponse resp = ODAP.GetDataTable(qry, DatabaseTarget.Adhoc);
            if (resp.Successful)
            {
                foreach (DataRow dr in resp.ResultsTable.Rows)
                {
                    auditResultTypes.Add(new AuditResultType(dr));
                }
            }
            m_all = auditResultTypes.ToArray();
        }
        ...
        public static AuditResultType Find(char value)
        {
            AuditResultType auditResultType = null;
            foreach (var eType in All)
            {
                if (eType.Value.Equals(value))
                {
                    auditResultType = eType;
                    break;
                }
            }
        ...
        public static string GetDescription(char value)
        {
            ...
            AuditResultType eType = Find(value);
            if (eType != null)
            {
        ...
        public static AuditResultType Deficiency { get { return Find('F'); } }

        /// <summary>Gets the emission type for the 'OBD' emission type record.</summary>
        public static AuditResultType Pass { get { return Find('P'); } }
    ...
    public class AuditResultType
    {
        ...
        public AuditResultType(DataRow dr)
        {
            m_value = NullSafe.ToString(dr["AUDIT_RESULT_CODE"])[0];
            m_description = NullSafe.ToString(dr["AUDIT_RESULT_DESC"]);
        }
