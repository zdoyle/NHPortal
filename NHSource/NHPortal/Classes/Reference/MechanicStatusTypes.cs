...
{
    /// <summary>Used for storing and accessing values stored in the r_MechStatType table.</summary>
    public static class MechanicStatusTypes
    {
        ...
        public static void Initialize()
        {
            string qry = "SELECT ret.CODE_VALUE, ret.DESCRIPTION" + Environment.NewLine
                       + "FROM   R_MECHANIC_STATUS ret";

            List<MechanicStatusType> MechStatTypes = new List<MechanicStatusType>();
            OracleResponse resp = ODAP.GetDataTable(qry, DatabaseTarget.Adhoc);
            if (resp.Successful)
            {
                foreach (DataRow dr in resp.ResultsTable.Rows)
                {
                    MechStatTypes.Add(new MechanicStatusType(dr));
                }
            }
            m_all = MechStatTypes.ToArray();
        }
        ...
        public static MechanicStatusType Find(char value)
        {
            MechanicStatusType MechStatType = null;
        ...
        public static string GetDescription(char value)
        {
            ...
            MechanicStatusType eType = Find(value);
            if (eType != null)
            {
        ...
        private static MechanicStatusType[] m_all;
        /// <summary>Gets all known emission types retrieved from the database.</summary>
        public static MechanicStatusType[] All
        {
            get
            {
                if (m_all == null)
        ...
        public static MechanicStatusType Active { get { return Find('A'); } }


        public static MechanicStatusType Inactive { get { return Find('I'); } }


        public static MechanicStatusType DisabledAdmin { get { return Find('D'); } }
        public static MechanicStatusType DisabledOther { get { return Find('O'); } }    
    }



    /// <summary>Stores a emission type record from the R_MechStatType table.</summary>
    public class MechanicStatusType
    {
        /// <summary>Instantiates a new instance of the MechanicStatusType class.</summary>
        /// <param name="dr">DataRow containing record information from the database.</param>
        public MechanicStatusType(DataRow dr)
        {
