...
{
    public class CountyStationRejectionData : IReportData
    {
        public CountyStationRejectionData()
        {
            BaseReport = BaseReportMaster.CountyStationRejection;
        }
        ...
        public void SetPropertyValues(DataRow dRow)
        {
            ...
            County = NullSafe.ToString(dRow[i++]);
        ...
        public void AddDataToReportRow(ReportRow currentRow)
        {
            ...
            // % Reject
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(OverallReject, TestCount));
            // Safety Count
            currentRow.Cells.Add(SafetyCount);
            // Safety Reject
            currentRow.Cells.Add(SafetyReject);
            // % Safety Reject
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(SafetyReject, TestCount));
            // OBD Count
            currentRow.Cells.Add(OBDCount);
            // OBD Reject
            currentRow.Cells.Add(OBDReject);
            // % OBD Reject
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(OBDReject, TestCount));
            // OBD NoComm
            currentRow.Cells.Add(OBDNoComm);
            // % OBD NoComm
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(OBDNoComm, TestCount));
            // Visual Count
            currentRow.Cells.Add(VisualCount);
            // Visual Reject
            currentRow.Cells.Add(VisualReject);
            // % Visual Reject
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(VisualReject, TestCount));
            // Abort Count
            currentRow.Cells.Add(OverallAbort);
            // % Abort Count
            currentRow.Cells.Add(BaseReportMaster.CalculatePercent(OverallAbort, TestCount));
        }
        ...
        public void SetTotalRowValues(ReportRow totalRow)
        {
            ...
            // % Reject
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalOverallReject, TotalTestCount));
            // Safety Count
            totalRow.Cells.Add(TotalSafetyCount);
            // Safety Reject
            totalRow.Cells.Add(TotalSafetyReject);
            // % Safety Reject
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalSafetyReject, TotalTestCount));
            // OBD Count
            totalRow.Cells.Add(TotalOBDCount);
            // OBD Reject
            totalRow.Cells.Add(TotalOBDReject);
            // % OBD Reject
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalOBDReject, TotalTestCount));
            // OBD NoComm
            totalRow.Cells.Add(TotalOBDNoComm);
            // % OBD NoComm
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalOBDNoComm, TotalTestCount));
            // Visual Count
            totalRow.Cells.Add(TotalVisualCount);
            // Visual Reject
            totalRow.Cells.Add(TotalVisualReject);
            // % Visual Reject
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalVisualReject, TotalTestCount));
            // Abort Count
            totalRow.Cells.Add(TotalOverallAbort);
            // % Abort Count
            totalRow.Cells.Add(BaseReportMaster.CalculatePercent(TotalOverallAbort, TotalTestCount));
        }
        ...
        public string BuildSQL()
        {
            ...
            builder.Append(" SELECT DISTINCT COUNTY, STATIONID,");
            builder.Append(String.Format(BaseReportMaster.QUERY_REPORT, "STATIONID, COUNTY"));
            builder.Append("{0}");
            builder.AppendLine("ORDER BY COUNTY, DECODE((OVERALL_PASS + OVERALL_REJECT + OVERALL_CORRECTED),0,0,OVERALL_REJECT/(OVERALL_PASS + OVERALL_REJECT + OVERALL_CORRECTED)) DESC, STATIONID ");
